...
void Net::backProp(const vector<double> &targetVals) {
	//Calculate overall net error (Root Mean Square error or RMS error) of output neuron errors
...
Net::Net(const vector<unsigned> &topology) {	
	...
	for (unsigned layerNum = 0; layerNum < numLayers; ++layerNum) {
		...
		//forst the bias node's output value to be 1.0. It's the last neuron created above
...
int main() {
	...
	for (unsigned x = 0; x < resultVals.size(); ++x) {
		cout << resultVals[x] << endl;
	}
}